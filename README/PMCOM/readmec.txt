...
SAVEWAIT	ON / OFF		可以設定是否要將等到的字串行存在變數中,以供解析字串使用,需要
					配合WAIT來使用
					SAVEWAIT ON   打開等待字串行存檔
					WAIT GOOD = MYDATA   會將等到GOOD字串的那一行存到VARINI中的varstring
							     變數名稱為 MYDATA 的資料中
					SAVEWAIT OFF     關掉等待字串行的存檔
					如果超過等待時間則會在 MYDATA中存FALSE
...
COMSTR		DATA			DATA有這個參數是存在com.INI中的IMAGE section 中的名稱字串
					將這個Section 中key 值等於 DATA的字串取出以COM PORT 將其    
					送出
					...
					COMSTR XLDR
					會產生執行下列的命令,也就是由COM PORT 送出 XPEN36A3.bin 字串
...
FORMID		NON			會記錄目前的FORM的Caption 號碼,可使用 ECHO = FORMID 顯示出來其Caption 號碼
					可運用在CASE FORMID等功能中.